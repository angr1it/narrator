...
Чтобы `chunk_mentions.j2` было универсально и подключалось по флагу, стоит вынести его подключение из логики шаблона в конфигурацию:

- поле `use_base: bool = True` в `CypherTemplateBase` позволяет задать, нужно ли оборачивать доменный шаблон в базовую структуру.
- при рендеринге (в `render()` или TemplateService) можно автоматически подставлять `chunk_mentions.j2` как основной шаблон, а доменный шаблон — как внутренний блок `include`.
...
- ✅ `cypher: str` в модели `CypherTemplateBase` теперь может быть переопределён через `use_base: True`, автоматически подставляя `chunk_mentions.j2`.
