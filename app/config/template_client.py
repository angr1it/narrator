...
class TemplateServiceClient:
    ...
    def __init__(self, address: str) -> None:
        ...
        self._stub = template_service_pb2_grpc.TemplateServiceStub(self._channel)

    async def find_templates(
        self, query: str, *, category: str | None = None, top_k: int = 10
    ) -> List[CypherTemplate]:
        request = template_service_pb2.FindTemplatesRequest(query=query, top_k=top_k)
        if category:
    ...
    @staticmethod
    def _pb_to_model(msg: common_pb2.Template) -> CypherTemplate:
